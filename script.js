...
<head>
  ...
  <!-- כולל את הפונקציה loadScript בתוך הקובץ -->
  <script>
    function loadScript(src) {
      return new Promise(function(resolve, reject) {
        var script = document.createElement('script');
  ...
  <style>
    ...
    .signature-buttons, .export {
      margin-top: 10px;
    }
    .signature-buttons button, .export button {
      background: #2980b9;
      color: #fff;
      padding: 8px 12px;
      border: none;
...
<body>
  <div class="proposal-container" id="proposal-container">
    <div class="header">
      <h1><span class="brand">Improve-IT</span> - הצעת מחיר למוצרי הדרכה דיגיטליים עבור ${companyName}</h1>
    </div>
    <div class="content">
      <p><strong>לכבוד:</strong> ${companyName}</p>
      <p><strong>איש קשר:</strong> ${contactName}</p>
      <p><strong>תפקיד:</strong> ${contactPosition}</p>
      <p><strong>תאריך:</strong> ${currentDate}</p>
      
      <h2>פרופיל חברה</h2>
      <p><span class="brand">Improve-IT</span> הוקמה בשנת 2010 על ידי זיו גלבוע, מומחה בעל ניסיון ניהולי וייעוצי בארגונים מובילים.
      החברה מתגאה בצוות מקצועי הכולל 10 מפתחי הדרכה מנוסים, מפיקים, גרפיקאים, מנחים ויועצים.
      אנו משלבים מתודולוגיה וטכנולוגיה מתקדמת לשיפור ביצועים ולהפיכת הארגון לארגון לומד.</p>
      
      <h2>רקע</h2>
      <p>${companyName} שוקלת לשלב לומדות מדף במערכת LMS בשנת ${proposalYear}, במטרה לשפר את תהליכי ההדרכה וההכשרה.
      ההצעה נועדה להתאים את ההדרכה לצרכים המשתנים של העובדים ולשפר את ביצועי הארגון.</p>
      
      <h2>הפתרון המוצע - כללי</h2>
      <p>הפתרון המוצע מתמקד בלומדות מדף ייעודיות לכלל עובדי ${companyName}.
      הלומדות כוללות אלמנטים אינטראקטיביים כגון סימולציות, תרגולים והדגמות של נהלי עבודה,
      המאפשרים לעובדים לנתח מקרים ולהתמודד עם אתגרי יום-יום בהתאם לתהליכי העבודה והנחיות הארגון.</p>
    ...
    <div class="footer">
      <p>© ${proposalYear} <span class="brand">Improve-IT</span>. כל הזכויות שמורות.</p>
    </div>
  </div>
  <script>
    const sigDateElem = document.getElementById('signature-date');
    if (sigDateElem) {
      sigDateElem.value = new Date().toLocaleDateString('he-IL');
    }
    let signaturePad;
  ...
  link.download = `הצעת_מחיר_${Date.now()}.html`;
